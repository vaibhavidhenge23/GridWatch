...
interface Alert {
  ...
  opened_at: string; suppressed: boolean
}
    ...
    <div className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onBack} className="text-sm text-gray-400 hover:text-white">← Dashboard</button>
        </div>
            ...
            {alerts.map(alert => (
              <div key={alert.id} className="bg-gray-900 border border-gray-800 rounded-lg p-4 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  <span className={`text-xs font-medium ${severityColor[alert.severity]}`}>{alert.severity.toUpperCase()}</span>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <span className={`text-xs px-2 py-0.5 rounded ${statusColor[alert.status]}`}>{alert.status}</span>
                  <span className="text-xs text-gray-500">{new Date(alert.opened_at).toLocaleString()}</span>
